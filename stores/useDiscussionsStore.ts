...
export const useDiscussionsStore = create<DiscussionsState>((set, get) => ({
  ...
  fetchDiscussion: async (id: string) => {
    ...
    try {
      ...
      if (user) {
        ...
        data.is_bookmarked = bookmarks && bookmarks.length > 0;
      }
  ...
  createDiscussion: async (data: {
    ...
    try {
      ...
      const { tags, ...discussionData } = data;
      ...
      if (tags && tags.length > 0) {
        try {
          // You might need to create a separate tags table and relation
          // or store as JSON in a separate field
          console.log('Tags are not supported in the current schema:', tags);
        } catch (tagError) {
